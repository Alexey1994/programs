...
void enum_FAT_data(
	...
	Boolean(*on_data)(FAT_File_System* file_system, FAT_Data* data, Byte* name, Number8 data_offset, Byte* context),
	Byte* context
)
{
	Byte      sector[512];
...
/*
static Boolean print_FAT_data(FAT_File_System* file_system, FAT_Data* data, Byte* name)
{
	print("wc", name, '\n');
	return 1;
}
...
static Boolean on_FAT_data_find(FAT_File_System* file_system, FAT_Data* data, Byte* name, Number8 data_offset, FAT_Find_Context* context)
{
	if(!compare_null_terminated_bytes(context->data_name, name))
	{
		context->finded = 1;
...
static Boolean open_FAT_data(FAT_File_System* file_system, Byte* data_name, Number32 data_cluster, FAT_Data* data)
{
...
Boolean open_FAT_directory(FAT_File_System* file_system, Byte* directory_name)
{
	FAT_Data directory;
...
Boolean open_FAT_file(FAT_File_System* file_system, Byte* directory_name, FAT_Data* file)
{
	FAT_Data directory;
...
Boolean allocate_FAT_data(FAT_File_System* file_system, Byte* name, FAT_Data_Attribute attributes, Number32 cluster_number, Number32 file_size)
{
	Byte      sector[512];
	FAT_Data* data;
	Number32  data_cluster;
...
Boolean create_FAT_directory(FAT_File_System* file_system, Byte* name)
{
...
Boolean create_FAT_file(FAT_File_System* file_system, Byte* name, Byte* bytes, Number32 size_of_bytes)
{
	Number32 number_of_clusters;
	Number32 cluster_number;
	Number32 i;
