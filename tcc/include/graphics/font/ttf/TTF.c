...
Boolean set_TTF_header_position(Reader* reader, Number16 number_of_tables, Byte* header_name)
{
	...
	for(i = 0; i < number_of_tables; ++i)
	{
		...
		{
			set_file_position(reader, offset);
			return 1;
		}
	}
...
void TTF_to_HTML(TTF* ttf, Number16* HTML_file_name)
{
	TTF_Glyph*         glyph;
...
void main()
{
	...
	FILE_READER(L"arial.ttf")
		read_TTF(&ttf, &reader);
	END_READER

	TTF_to_HTML(&ttf, L"a.html");	
}