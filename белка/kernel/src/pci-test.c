...
/*
	...
	print(message, device->config.vendor, ' ', device->config.device, '\n');
	...
	for(i = 0; i < 6; ++i)
	{
		...
		if(space_required)
		{
			...
			print(message, i, ' ', space_required, '\n');
		}
	}
}
...
void test_PCI_device(PCI_Device* device)
{
	if(device->config.vendor != 0x1002 || device->config.device != 0x9715)
		return;

	//print("s", "Hi!\n");
	print_PCI_device(device);

	Number32* framebuffer = get_PCI_MMIO_address(device, 0);
	Number32 io = get_PCI_IO_address(device, 1);
	Number32 mmio = get_PCI_MMIO_address(device, 2);
