...
static void format_FAT12(File storage_file, Number64 storage_file_size)
{
	Byte   sector[512];
	FAT12* fat12;
...
static void format_FAT32(File storage_file, Number64 storage_file_size)
{
	Byte   sector[512];
	FAT32* fat32;
	Number i;
...
static void read_FAT_sector(FAT_File_System* file_system, Number32 cluster_number, Byte* sector, Number32 offset)
{
...
static void write_FAT_sector(FAT_File_System* file_system, Number32 cluster_number, Byte* sector, Number32 offset)
{
	set_file_position(file_system->storage_file, (file_system->data_start_sector + (cluster_number - 2) * file_system->sectors_in_cluster + offset) * 512);
...
static void open_FAT_File_System(FAT_File_System* file_system, File storage_file)
{
	Byte     sector[512];
	FAT32*   fat32;
...
static Number32 next_FAT_cluster(FAT_File_System* file_system, Number32 current_cluster)
{
...
static Number32 allocate_FAT_cluster(FAT_File_System* file_system)
{
...
static Number32 allocate_next_FAT_cluster(FAT_File_System* file_system, Number32 current_cluster)
{
...
static void list_FAT_directory(FAT_File_System* file_system)
{
	enum_FAT_data(
		file_system,
...
static Boolean open_FAT_directory(FAT_File_System* file_system, Number16* directory_name)
{
	FAT_Data directory;
...
static Boolean open_FAT_file(FAT_File_System* file_system, Number16* directory_name, FAT_Data* file)
{
	FAT_Data directory;
...
static Boolean allocate_FAT_data(FAT_File_System* file_system, Number16* name, FAT_Data_Attribute attributes, Number32 cluster_number, Number32 file_size)
{
	Byte      sector[512];
	FAT_Data* data;
	Number32  data_cluster;
...
static Boolean update_FAT_data(FAT_File_System* file_system, FAT_Data* data)
{

}


static Boolean create_FAT_directory(FAT_File_System* file_system, Number16* name)
{
...
static Boolean create_FAT_file(FAT_File_System* file_system, Number16* name, Byte* bytes, Number32 size_of_bytes)
{
	Number32 number_of_clusters;
	Number32 cluster_number;
	Number32 i;
