...
//GUI Block Memory Mapped Registers. These registers are FIFOed.
...
#define SET(register_number, value) *(Number32*)(mmio + register_number) = (value)
#define GET(register_number) *(Number32*)(mmio + register_number)


typedef struct
{
	// CRTC registers
	Number32 crtc_gen_cntl;
	Number32 crtc_h_total_disp;
	Number32 crtc_h_sync_strt_wid;
	Number32 crtc_v_total_disp;
	Number32 crtc_v_sync_strt_wid;
	Number32 crtc_pitch;
	...
	// Computed PLL values
	Number32 feedback_div;
	Number32 post_div;

	// PLL registers
	Number32 ppll_ref_div;
	Number32 ppll_div_3;
}
Display_Params;


//https://github.com/haiku/haiku/blob/master/src/add-ons/accelerants/ati/rage128_mode.cpp

/*
void set_PLL(Number32 mmio, Number8 index, Number32 value);
Number32 get_PLL(Number32 mmio, Number8 index);
void write_PLL_update(Number32 mmio);*/

void turn_off_display(Number32 mmio);


void configure_ATI_128_video(PCI* pci, PCI_Device* device, Number module_address)
{
	Byte message[] = {'n', 'c', 0};
	pci->kernel->print(message, 1002, '\n');
	//pci->kernel->print(module_address + "n", 8086);

	Number i;

	Number32* framebuffer = pci->get_PCI_MMIO_address(device, 0);
	Number32 io = pci->get_PCI_IO_address(device, 1);
	Number32 mmio = pci->get_PCI_MMIO_address(device, 2);

	{
		Byte message[] = {'n', 0};
		pci->kernel->print(message, *(Number32*)(mmio + CNFG_MEMSIZE));
	}
...
/*
	...
	SET(OVR_CLR, 0);
	SET(OVR_WID_LEFT_RIGHT, 0);
	SET(OVR_WID_TOP_BOTTOM, 0);
	SET(OV0_SCALE_CNTL, 0);
	SET(MPP_TB_CONFIG, 0);
	SET(MPP_GP_CONFIG, 0);
	SET(SUBPIC_CNTL, 0);
	SET(VIPH_CONTROL, 0);
	SET(I2C_CNTL_1, 0);
	SET(GEN_INT_CNTL, 0);
	SET(CAP0_TRIG_CNTL, 0);
	SET(CAP1_TRIG_CNTL, 0);

	Number32 bus_cntl = GET(BUS_CNTL);
	if(bus_cntl & (R128_BUS_WRT_BURST | R128_BUS_READ_BURST))
		SET(BUS_CNTL, bus_cntl | R128_BUS_RD_DISCARD_EN | R128_BUS_RD_ABORT_EN);

	SET(DDA_CONFIG, params.dda_config);
	SET(DDA_ON_OFF, params.dda_on_off);

	SET(CRTC_GEN_CNTL, params.crtc_gen_cntl);

	SET(CLOCK_CNTL_INDEX, GET(CLOCK_CNTL_INDEX) | R128_PLL_DIV_SEL);

	set_PLL(mmio, VCLK_ECP_CNTL, get_PLL(mmio, VCLK_ECP_CNTL) | R128_VCLK_SRC_SEL_CPUCLK);

	set_PLL(mmio, PPLL_CNTL, get_PLL(mmio, PPLL_CNTL) | PPLL_ATOMIC_UPDATE_EN | PPLL_VGA_ATOMIC_UPDATE_EN);

	write_PLL_update(mmio);*/

	Number32 format = 6;//true color

	Number32 h_total = 1024;
	Number32 h_display = 1024;
	Number32 v_total = 768;
	Number32 v_display = 768;
	Number32 crtc_h_total_disp = (((h_total / 8) - 1) & 0xffff) | (((h_display / 8) - 1) << 16);
	Number32 crtc_v_total_disp = ((v_total - 1) & 0xffff) | ((v_display - 1) << 16);

	SET(CRTC_H_TOTAL_DISP, crtc_h_total_disp);
	SET(CRTC_V_TOTAL_DISP, crtc_v_total_disp);
	SET(CRTC_V_SYNC_STRT_WID, 0);
	SET(CRTC_H_SYNC_STRT_WID, 0);
	SET(CRTC_OFFSET, 0);
	SET(CRTC_OFFSET_CNTL, 0);
	SET(CRTC_PITCH, h_display / 8);

	SET(CRTC_GEN_CNTL, CRTC2_EXT_DISP_EN | CRTC2_EN | (format << 8)); //set true color
...
void set_PLL(Number32 mmio, Number8 index, Number32 value)
{
	*(Number8*)(mmio + CLOCK_CNTL_INDEX) = (index & 0x3f) | PLL_WR_EN;
	*(Number32*)(mmio + CLOCK_CNTL_DATA) = value;
}


Number32 get_PLL(Number32 mmio, Number8 index)
{
	*(Number8*)(mmio + CLOCK_CNTL_INDEX) = index & 0x3f;
	return *(Number32*)(mmio + CLOCK_CNTL_DATA);
}


void write_PLL_update(Number32 mmio)
{
	while(get_PLL(mmio, PPLL_REF_DIV) & PPLL_ATOMIC_UPDATE_R);

	set_PLL(mmio, PPLL_REF_DIV, get_PLL(mmio, PPLL_REF_DIV) | PPLL_ATOMIC_UPDATE_W);
}*/


void turn_off_display(Number32 mmio)
{
	SET(CRTC_EXT_CNTL, GET(CRTC_EXT_CNTL) | R128_CRTC_DISPLAY_DIS | R128_CRTC_HSYNC_DIS | R128_CRTC_VSYNC_DIS);
}