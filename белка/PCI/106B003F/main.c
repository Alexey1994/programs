...
void configure_USB_OHCI_host(PCI* pci, PCI_Device* device, Number module_address)
{
	//Byte message[] = {'n', 's', 0};
	...
	/*pci->kernel->print(M("ns"), interface->revision, M(" "));
	pci->kernel->print(M("ns"), interface->status, M("\n"));
	pci->kernel->print(M("ns"), interface->control, M("\n"));*/
